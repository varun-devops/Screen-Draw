"""
Build script for creating a Windows installer for WebtroopsScreen Draw v1.0.1
"""
...
def create_inno_setup_script():
    ...
    # Create the installer script content using triple quotes to avoid escaping issues
    script_content = f'''
...
def build_installer():
    ...
    if not os.path.exists(iscc_path):
        ...
        print("Please install Inno Setup or update the path in this script.")
        return False
...
def main():
    ...
    try:
        ...
        if success:
            print(f"\nBuild successful! Installer is available in the Output directory.")
            # Copy the installer to the Output directory
            if not os.path.exists("Output"):
                os.makedirs("Output")
            installer_name = f"{APP_NAME}_Setup_{APP_VERSION}.exe"
            source_path = os.path.join(OUTPUT_DIR, installer_name)
            target_path = os.path.join("Output", installer_name)
